...
## Screenshots of the model being used in a simulation
![Model passed](model/pass.png)
![Model failed](model/fail.png)

## Observation
Based on the simulated csv dataset, the evaluation of 108 distinct student profiles reveals a perfectly balanced performance distribution, with exactly 50% of scenarios resulting in a passing grade and 50% resulting in failure. Performance is most heavily influenced by attendance, assignment scores, and quiz scores, where achieving the highest marks in any of these categories (Good attendance, an "A" on assignments, or High quiz scores) grants a student an overwhelmingly strong 77.8% chance of passing. Conversely, falling into the lowest tier in any of these areas (Poor attendance, a "C", or Low quiz scores) pulls the pass rate down drastically to just 22.2%, making failure highly likely. For students who fall into the middle of the pack—earning "B"s, scoring "Mid" on quizzes, or maintaining "Average" attendance—the outcome becomes a perfect 50/50 split, meaning their success ultimately relies on their behavioral commitment. In these tie-breaker scenarios, study habits become the deciding factor, as students who dedicate "Long" study hours enjoy a strong overall 63% pass rate compared to the 37% pass rate of those who study short hours.
